import { eq, and, desc, avg, count, sum, gte, lte } from "drizzle-orm";
...
async function _fetchUserAnalyticsFromDb(userId: string, dateRange?: { from?: Date; to?: Date }): Promise<UserAnalytics> {
  ...
  let query = db
    ...
    .where(eq(interviewsTable.userId, userId)) as any; // Type assertion to bypass complex type checking
  
  // Apply date range filters if provided
  if (dateRange?.from && dateRange?.to) {
    query = query.where(
      and(
        gte(interviewResultsTable.createdAt, dateRange.from),
        lte(interviewResultsTable.createdAt, dateRange.to)
      )
    ) as any;
  } else if (dateRange?.from) {
    query = query.where(gte(interviewResultsTable.createdAt, dateRange.from)) as any;
  } else if (dateRange?.to) {
    query = query.where(lte(interviewResultsTable.createdAt, dateRange.to)) as any;
  }

  const results = await query.orderBy(desc(interviewResultsTable.createdAt));
  ...
  const avgOverallScore = results.reduce((sum: number, r: any) => sum + (parseFloat(r.overallScore.toString())), 0) / totalInterviews;
  const avgTechnicalScore = results.reduce((sum: number, r: any) => sum + (parseFloat(r.technicalScore.toString())), 0) / totalInterviews;
  const avgCommunicationScore = results.reduce((sum: number, r: any) => sum + (parseFloat(r.communicationScore.toString())), 0) / totalInterviews;
  const avgProblemSolvingScore = results.reduce((sum: number, r: any) => sum + (parseFloat(r.problemSolvingScore.toString())), 0) / totalInterviews;
  const avgConfidenceScore = results.reduce((sum: number, r: any) => sum + (parseFloat(r.confidenceScore.toString())), 0) / totalInterviews;
  
  const totalQuestionsAnswered = results.reduce((sum: number, r: any) => sum + (r.questionsAnswered || 0), 0);
  const totalQuestionsSkipped = results.reduce((sum: number, r: any) => sum + (r.questionsSkipped || 0), 0);
  ...
  const trendData = results.map((r: any) => ({
    date: r.createdAt.toISOString().split('T')[0] || new Date().toISOString().split('T')[0], // Ensure date is always a string
  ...
  for (const category of allCategories) {
    const categoryResults = results.filter((r: any) => {
      const strengths = r.strengths && Array.isArray(r.strengths) ? r.strengths : [];
      const weaknesses = r.weaknesses && Array.isArray(r.weaknesses) ? r.weaknesses : [];
      
      return strengths.includes(category) || weaknesses.includes(category);
    });

    if (categoryResults.length > 0) {
      const avgScore = categoryResults.reduce((sum: number, r: any) => {
